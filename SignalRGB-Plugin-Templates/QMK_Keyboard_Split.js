...
const Keyboard = {
	Left : {
		...
		lednames: [
			...
			"Left 9", "Left 10", "Left 11", "Left 12",
		],
		ledpos:	[
			[0, 0], [1, 0], [2, 0], [3, 0],
			[0, 1], [1, 1], [2, 1], [3, 1]
...
function grabColors(overrideColor) {
	...
	for(let iIdx = 0; iIdx < Object.keys(Keyboard).length; iIdx++){
		const Side = Object.values(Keyboard)[iIdx];

		for(let i = 0; i < Object.values(Keyboard)[iIdx].ledpos.length; i++) {
			const iPxX = Side.ledpos[i][0];
			const iPxY = Side.ledpos[i][1];
			let color;
			...
			if(overrideColor) {
				...
				if(iIdx == 0){
					color = device.subdeviceColor("LeftSide", iPxX, iPxY);
				}else{
					color = device.subdeviceColor("RightSide", iPxX, iPxY);
				}
			}

			const iLedIdx 		= Side.ledids[i] * 3;
